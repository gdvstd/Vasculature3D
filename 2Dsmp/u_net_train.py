...
root_path="/"
parhs=["/train/train/kidney_1_dense"]
for i,path in enumerate(parhs):
    if path=="/train/train/kidney_3_dense":
        continue
...
path1="/train/train/kidney_3_sparse"
path2="/train/train/kidney_3_dense"
...
for epoch in range(CFG.epochs):
    ...
    for thr in np.arange(0.1, 1, 0.1):
        ...
        if best_thr_val_score < val_scores:
            best_thr_val_score = val_scores
            best_thr = thr

    time.close()
    tc.save(model.module.state_dict(),f"./{CFG.backbone}_{epoch}_loss{losss:.2f}_score{scores:.2f}_val_loss{val_losss:.2f}_val_score{best_thr_val_score:.2f}_threshold{best_thr}_midd_1024.pt")
    
    if  best_score < best_thr_val_score:
        best_score = best_thr_val_score
        best_epoch = epoch
    
    print(f"Best score until {epoch}: {best_score}, best epoch: {best_epoch}")
